...
namespace FundooNotesApp.API.Controller
{
    ...
    public class NoteController : ControllerBase
    {
        ...
        [HttpGet("search")]
        public IActionResult SearchNotes([FromQuery] NoteSearchFilterDTO filterDTO)
        {
            try
            {
                // 1. Safely extract the current authenticated User ID from JWT Claims
                int userId = LoggedInUserId();


                 // 2. Execute the reusable search routing service
                var results = _service.SearchAndFilterNotes(userId, filterDTO);

                // 3. Return results
                if (results.Count == 0)
                {
                    return Ok(new { message = "No matching notes found.", data = results });
                }
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { message = "An error occurred while processing your search request." });
            }
        }
